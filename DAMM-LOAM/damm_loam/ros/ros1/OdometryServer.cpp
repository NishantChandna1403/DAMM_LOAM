...
OdometryServer::OdometryServer(const ros::NodeHandle &nh, const ros::NodeHandle &pnh)
    ...
    // Construct the main damm-loam odometry node
    // Create DammConfig struct from our parameters
    damm_loam::pipeline::DammConfig damm_config;
    damm_config.max_range = config_.max_range;
    damm_config.min_range = config_.min_range;
    damm_config.deskew = config_.deskew;
    damm_config.voxel_size = config_.voxel_size;
    damm_config.map_cleanup_radius = config_.map_cleanup_radius;
    damm_config.planarity_threshold = config_.planarity_threshold;
    damm_config.max_points_per_voxel = config_.max_points_per_voxel;
    damm_config.desired_num_voxelized_points = config_.desired_num_voxelized_points;
    damm_config.initial_threshold = config_.initial_threshold;
    damm_config.min_motion_th = config_.min_motion_th;
    damm_config.max_num_iterations = config_.max_num_iterations;
    damm_config.convergence_criterion = config_.convergence_criterion;
    
    // Initialize odometry with the config
    odometry_ = damm_loam::pipeline::damm_loam(damm_config);
    ...
    odom_publisher_ = pnh_.advertise<nav_msgs::Odometry>("/damm_loam/odometry", queue_size_);
    traj_publisher_ = pnh_.advertise<nav_msgs::Path>("/damm_loam/trajectory", queue_size_);
    if (publish_debug_clouds_) {
        map_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/local_map", queue_size_);
        ground_points_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/ground_points", queue_size_);
        roof_points_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/roof_points", queue_size_);
        wall_points_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/wall_points", queue_size_);
        edge_points_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/edge_points", queue_size_);
        non_planar_points_publisher_ = pnh_.advertise<sensor_msgs::PointCloud2>("/damm_loam/non_planar_points", queue_size_);
    }
    // Initialize the transform buffer
    tf2_buffer_.setUsingDedicatedThread(true);
    path_msg_.header.frame_id = odom_frame_;

    ROS_INFO("damm-loam ROS 1 Odometry Node Initialized");
}
...
void OdometryServer::RegisterFrame(const sensor_msgs::PointCloud2::ConstPtr &ground_msg,
    ...
    // Register frame, main entry point to damm-loam pipeline
    try {
        ...
        ROS_INFO("damm-loam registration complete");

        // Compute the pose using damm, ego-centric to the LiDAR
        const Sophus::SE3d damm_pose = odometry_.poses().back();
        ...
        ROS_ERROR("Exception in damm-loam pipeline: %s", e.what());
    }
}
...
void OdometryServer::PublishClouds(const ros::Time &stamp,
    ...
    map_publisher_.publish(*EigenToPointCloud2(damm_map, odom_header));
}

}  // namespace damm_loam_ros

int main(int argc, char **argv) {
    ros::init(argc, argv, "damm_loam");
    ros::NodeHandle nh;
    ros::NodeHandle nh_private("~");

    damm_loam_ros::OdometryServer node(nh, nh_private);
